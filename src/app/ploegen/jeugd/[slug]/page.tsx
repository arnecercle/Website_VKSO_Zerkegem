...
export default async function YouthTeamPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const team = youthTeams.find((t) => t.slug === slug);
  if (!team) notFound();
          ...
          {team.staff.length > 0 && (
            <div className="mx-auto flex max-w-3xl flex-wrap justify-center gap-8">
              {team.staff.map((member) => (
                <div key={member.name || member.role} className="flex flex-col items-center text-center">
                  {member.image && (
                  ...
                  {member.name && <p className="font-archivo text-white/90">{member.name}</p>}
                  {member.email && (
