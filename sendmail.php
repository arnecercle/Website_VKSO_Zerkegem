...
function getTopText ($replyTo) {
    $topText = "You received a new message sent via the contact form on vksozerkegem.be.";
    $topText = strlen($topText) > 0 ? $topText : "You received a new message from __REPLYTO__ sent via the contact form on vksozerkegem.be.";
...
function getMailHeaders ($from, $replyTo, $boundary) {
    $headers = array(
        ...
        "Message-ID: <" . implode("@", [md5(uniqid(time())), "vksozerkegem.be"]) . ">",
        "Date: ".date("r (T)"),
        "X_Mailer: PHP/" . phpversion(),
        "X-G1i-WSB-Sendmail: 129.1.27",
    );
...
function getDebugInfo() {
    ...
    if (isValidRequester() || $debugHeader === "vksozerkegem.be") {
...
function processMail() {
    ...
    if (empty($response)) {
        $contactFormEmails = json_decode('{"80AD81DF-D9F3-45EF-833A-4D67A3A6F772":"info@vksozerkegem.be","1A66DD14-A7AC-4BC0-BCFA-12FA4EB56E74":"info@vksozerkegem.be"}', true);
